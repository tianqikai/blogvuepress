...
### 9.5.2 PRIMARY  UNION  UNION RESULT
...
:::
### 9.5.4 DEPENDENT UNION、DEPENDENT SUBQUERY
...
### 9.13.1 No tables used 

当查询语句的没有 FROM 子句时将会提示该额外信息。 
### 9.13.2 Impossible WHERE 

查询语句的 WHERE 子句永远为 FALSE 时将会提示该额外信息。 
### 9.13.3 No matching min/max row 
...
### 9.13.7 Using join buffer (Block Nested Loop) 
...
### 9.13.9 Using intersect(...)、Using union(...)和 Using sort_union(...) 
...
### 9.13.13 Start temporary, End temporary 
